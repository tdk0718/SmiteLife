...
public class InventoryManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Instance = this;
        foreach (var e in itemRegistry)
            if (e != null && !string.IsNullOrEmpty(e.id) && e.data != null)
                _registry[e.id] = e.data;
        ItemCatalog.RegisterDefaults(this);
    }
    ...
    public IEnumerable<string> ItemIds => _registry.Keys;

    public void Add(string id, int qty = 1)
    {
        if (!_registry.ContainsKey(id) || qty <= 0) return;
    ...
    {
        var item = GetItem(id);
        if (item == null || !Has(id)) return false;

        if (item.edible)
        {
            Remove(id);
            StatsManager.Instance?.Eat(item.hungerAmount, item.hpAmount);
            if (item.slow) StatsManager.Instance?.ApplySlowEffect(15f);
            return true;
        }

        if (item.isTool)
        {
            SetEquipped(_equipped == id ? null : id);
            return true;
        }
